...
export class AuthService {
  ...
  async deptDel(id) {
    const res = await this.app.db.update('department', { isDel: 1 }).where('id', id).execute();
    if (!res.affectedRows) throw new Error('操作失败');
    // 保存日志信息
  ...
  async roleSave({ id, keys, deptIds, ...data }) {
    ...
    if (!res.affectedRows) throw new Error('操作失败');
    await this.log.insert(2, id ? `更新角色名称-${data.name}` : `新增角色-${data.name}`);
    return true;
  }

  async roleDel(id) {
    const res = await this.app.db.update('role', { isDel: 1 }).where('id', id).execute();
    if (!res.affectedRows) throw new Error('操作失败');
  ...
  async menuSave(data) {
    ...
    await this.log.insert(2, data.id ? `更新菜单-${data.name}` : `新增菜单-${data.name}`);
    return true;
  }
